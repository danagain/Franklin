...
def thread_work(market, num):
    ...
    while True:
        ...
        while current_state == "TrendingUp":
                ...
                latest_summary = bittrex.get_latest_summary()
                """
                #If you want to insta sell order uncomment this block

                bid = current_purchase * 1.20
                qty = balance
                ...
                if balance > 0 and market in good_coins and (latest_summary['Last'] * balance) >= (BTC_PER_PURCHASE * 1.3):
                        bid = latest_summary['Last']
                        bittrex.place_sell_order(bid)
                        current_state = "InitTrendingUp" #this will stop hunter buying the same thing and losing possibly multiple times
                if balance > 0 and market not in good_coins and (latest_summary['Last'] * balance) >= (BTC_PER_PURCHASE * 1.2):
                        bid = latest_summary['Last']
                        bittrex.place_sell_order(bid)
                        current_state = "InitTrendingUp" #this will stop hunter buying the same thing and losing possibly multiple times
                time.sleep(8)
